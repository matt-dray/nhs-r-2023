...
* It's the functions that R has before we install any packages.
:::
...
* You'll likely have recognised certain functions in there at very least.
:::
...
::: {.notes}
* Regardless of what you think of that style of coding, I want to make three points (and maybe a bonus one) to just remind you why base R is useful and may have changed since you last used it in anger.
* First, its stability over time and into the future.
* Second, that a lot of functionality exists in base R that could help to limit package dependencies.
* Third, that 'stability' does not mean 'unchanging' and 'unresponsive' to the needs and wants of the R community.
...
* And perhaps more famously, the conversion of the default `stringsAsFactors = TRUE` to `FALSE`, perhaps reflecting a change in R from a lanaguage of statistics (where character values as factors makes a lot of sense) to a more general-purpose language.
* Of course, there are still opportunities to improve base R. Lots of stuff can come out of the woodwork after 30 years, or as tastes change. Henrik Bengtsson has a GitHub repo to which people can add issues. Each one lists a specific gripe that someone has had with the functioning of the language. For example, out very own Tim Taylor at UKHSA posted about the behaviour of indexing a dataframe to return a single column. By default, a single column is returned as a vector; the user needs to add the argument `drop = FALSE` to retain the coulmnar structure. Perhaps that behaviour will updated in the future?
:::
